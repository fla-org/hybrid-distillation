...
def convert(
    ...
    model = AutoModelForCausalLM.from_config(config_obj, torch_dtype=precision)
    ...
    for i in range(config_obj.num_hidden_layers):
        ...
        if hasattr(model.model.layers[i].attn, 'q_norm'):
            print(f"llama.model.layers{i}.attn.q_norm.weight  -> model.model.layers{i}.attn.q_norm.weight")
            model.model.layers[i].attn.q_norm.weight.data.copy_(llama.model.layers[i].self_attn.q_norm.weight)
            torch.testing.assert_close(model.model.layers[i].attn.q_norm.weight, llama.model.layers[i].self_attn.q_norm.weight)
        if hasattr(model.model.layers[i].attn, 'k_norm'):
            print(f"llama.model.layers{i}.attn.k_norm.weight  -> model.model.layers{i}.attn.k_norm.weight")
            model.model.layers[i].attn.k_norm.weight.data.copy_(llama.model.layers[i].self_attn.k_norm.weight)
            torch.testing.assert_close(model.model.layers[i].attn.k_norm.weight, llama.model.layers[i].self_attn.k_norm.weight)
